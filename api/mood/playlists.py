...
def lambda_handler(event, context):
    """Handle mood playlists requests"""
    if event.get("httpMethod") == "OPTIONS":
        return cors_response({"ok": True}, 200)

    query_params = event.get("queryStringParameters") or {}
    mood_category = query_params.get("mood_category")

    if not mood_category:
        return cors_response({"error": "Mood category is required"}, 400)

    playlists = ytAnon.get_mood_playlists(mood_category)[:15]
    response = cors_response(playlists, 200)
